...
.get('/', function (req, res) {
    ...
    var now = new Date();
    _models.Item.findByIdAndUpdate(req.params.id, { $set: { 'borrows.$[currentborrower].returned': now } }, { arrayFilters: [{ 'currentborrower.returned': null, 'currentborrower.user': req.user._id }] }, function (err, docs) {
        console.log(err);
        if (err || !docs) return res.sendStatus(500);
        res.send(200);
    });
